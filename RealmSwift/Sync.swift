...
public extension SyncSession {
    /// Whether a given notifier block will report the upload status or the download status of its session.
    public enum NotifierDirection {
        /// :nodoc:
        case upload
        /// :nodoc:
        case download
    }
    /// How the notifier block will behave.
    public enum NotifierMode {
        /**
         The block will be called forever, or until the user unregisters it.
         The progress struct will always report the latest number of transferred bytes,
         and the latest number of total bytes to be transferred.
         */
        case alwaysReportLatest
        /**
         The block will, upon registration, store the total number of bytes
         to be transferred. The progress struct will always report the latest number
         ...
         number of bytes to be transferred, the block will be automatically
         unregistered.
         */
        case progressRelativeToFixedPoint
    }
    ...
    struct Progress {
        ...
        let bytesTransferred: UInt
        /// The total number of bytes that have been transferred, and are pending transfer.
        /// If the notifier is tracking downloads, this number represents the size of the
        /// changesets generated by all other clients using the database.
        /// If the notifier is tracking uploads, this number represents the size of the
        /// changesets representing the local changes on this client.
        let bytesPending: UInt

        /**
         The fraction of bytes transferred out of outstanding bytes to transfer.
         */
        var fractionTransferred: Double {
            let percentage = Double(bytesTransferred)/Double(bytesPending)
            return percentage > 1 ? 1 : percentage
        }

        var isTransferComplete: Bool {
            return bytesTransferred >= bytesPending
        }

        fileprivate init(transferred: UInt, pending: UInt) {
...
public extension SyncSession {
    ...
    struct Progress {
        ...
        private init(transferred: UInt, pending: UInt) {
            bytesTransferred = transferred
            bytesPending = pending
        }
    }
